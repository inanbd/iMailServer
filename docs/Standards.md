...
| MTA-STS | 8461 | **Implemented** | 11 | Both directions. **Checking** other domains' policies (`TransportPolicyProbe`, `MtaStsPolicyFetcher`) feeds the TLS category of the readiness report. **Publishing** this server's own is `MtaStsPolicyListener`: one route, `text/plain`, HTTPS on 443 with the certificate selected per handshake, 404 for everything else. §3.3 forbids senders following a redirect, so there is nothing to gain from another route. **Off by default and defaulting to `testing` when enabled** — §8.3 is blunt about the cost of a wrong `mx` list in `enforce` mode, and this is the only setting in the product that can stop mail that was arriving perfectly well. The `_mta-sts` record the DNS wizard proposes carries the id of the policy actually being served |
| TLS-RPT | 8460 | **Implemented** | 11 | The `_smtp._tls` record is checked (`tls.tls-rpt`) and proposed by the DNS wizard, and a submitted report is read and analysed: `TlsReportReader` parses §4's JSON and `TlsReport` groups the failures by result type, orders them by sessions lost and gives each a remedy — including for the types that are not this server's fault, since `dane-required` is the sender's policy and `dnssec-invalid` is the zone's signing. The parser is deliberately lenient about counts sent as strings and unknown result types, because the senders that matter most each differ from the ABNF in small ways and discarding their reports would lose the failures they also counted. **Collection is built**: `TlsReportCollector` makes a bounded pass over the mailbox the `rua` address delivers to, `TlsReportExtractor` walks the MIME tree for the attachment, transfer-decodes it, gunzips it under a ceiling and hands the JSON to the reader. It reads the mailbox and never changes it — nothing is marked, moved or deleted, because a human may be reading the same folder — and what has been examined is recorded separately, failures included, so a bounce is not re-opened on every pass forever. A report is filed under the domain whose `rua` address received it, never the policy-domain the sender wrote: taking the sender's word would let anyone who can reach the address file a report against any domain this server hosts. **Off by default**, since it needs the address published and a mailbox here to deliver into |
| REQUIRETLS | 8689 | Planned | Post-11 | Fails rather than downgrading when the policy cannot be met. **Moved out of Milestone 11**: it is an SMTP extension rather than a deliverability check — advertised on EHLO, requested per-message on `MAIL FROM`, and carried through the queue to the outbound client — so it touches the mail path rather than the report, and it is not among the deliverables `docs/Architecture.md` §27 lists for this milestone. Note the per-domain `RequireTlsForOutbound` setting is a **different thing**: a local policy for all mail to a domain, not RFC 8689's per-message negotiation |
...
| Standard | RFC | Status | Milestone | Notes |
|---|---|---|---|---|
| One-Click Unsubscribe | 8058 | **Not applicable (for now)** | — | **Reclassified out of Milestone 11, because it presumes a feature this product does not have.** RFC 8058 is a one-click endpoint for unsubscribing from a *mailing list*, and RFC 2369's headers say which list a message came from. This server has no list: it delivers mail for mailboxes and aliases, and there is nothing for a recipient to be unsubscribed from. Implementing them would mean inventing a bulk-sending feature first, which is a product decision rather than a standards gap. The row stays so the reasoning is on the record — if list sending is ever built, these two become required rather than optional, since major receivers now expect them from bulk senders |
| List-Unsubscribe header | 2369 | **Not applicable (for now)** | — | See the row above; the two stand or fall together |
...
| Reverse DNS / FCrDNS | 1912 (BCP) | **Implemented** | 11 | Checked and scored rather than merely documented: `IdentityChecks` resolves the public address's PTR names, resolves each of those forward again, and scores the round trip. An earlier revision of this row said Planned after the check had been built |
...
| Retry backoff, bounce, DSN, delay warning | RFC 3461/3464 (partial — see the Standards table), RFC 3834 (partial) | `OutboundDeliveryHostedServiceTests` — 11 tests including the exact retry interval, a permanent failure generating a DSN addressed to the original sender, a hosted sender's failure and delay-warning DSNs delivered locally while a remote sender's is still relayed, a null-reverse-path message never generating one, MX failures short-circuiting the delivery client entirely, maximum-lifetime expiry, and a delay-warning DSN sent once and never repeated |
