...
| Standard | RFC | Status | Milestone | Notes |
|---|---|---|---|---|
| IMAP4rev1 | 3501 | Partial | 10 | UID and UIDVALIDITY correctness is the priority. **Folder-name case matching diverges by provider** — exact on SQLite, case-insensitive on a default-collation SQL Server; see `docs/IMAP.md`. `CAPABILITY`, `NOOP`, `LOGOUT`, `STARTTLS`, `LOGIN`, `AUTHENTICATE`, `SELECT`, `EXAMINE`, `LIST`, `LSUB`, `STATUS`, `FETCH`, `STORE`, `EXPUNGE`, `CLOSE`, `CHECK`, `CREATE`, `DELETE`, `RENAME`, `SUBSCRIBE`, `UNSUBSCRIBE`, `COPY`, `APPEND`, `SEARCH` and `IDLE` are implemented; `FETCH` serves every data item §6.4.5 defines — the stored columns, `ENVELOPE`, `BODY`, `BODYSTRUCTURE`, every `BODY[…]` section including numbered MIME parts, the `RFC822*` equivalents, partials and the `.PEEK` forms — and all three macros. Everything else is answered with a tagged `NO` naming the command rather than pretended at. §6.4.6's last-paragraph SHOULD — an untagged `FETCH` when an external source changes a message's flags — **is met** for an idling session, through `ImapFlagWatch`. **The known conformance gap is literals.** §4.3 admits a literal wherever the grammar has an `astring`, and this server reads one only for `APPEND`: every other command is parsed from a single line, `ImapAstringReader` reports the `{n}` it saw rather than resolving it, and the handlers refuse that argument. So `SELECT {5}` draws a tagged `BAD`, and the octets the client then sends are parsed as a fresh command line. Clients use literals for non-ASCII mailbox names and `SEARCH` terms, which makes this a live interoperability risk against this milestone's exit criterion rather than a theoretical one |
...
| IMAP LITERAL- | 7888 | Partial | 10 | **`LITERAL-`, not `LITERAL+`.** RFC 7888 defines both; `LITERAL-` caps a non-synchronising literal at 4096 octets and `LITERAL+` places no bound on one, and §5 forbids advertising both. This product requires a hard cap — `{n+}` lets a client push bytes before the server can refuse — so it was never a `LITERAL+` server. `ImapLiteralSpecifier` parses the `{n+}` syntax and `ImapCapabilities` knows the atom. **`APPEND` does now read a literal's octets, synchronising and not** (`ImapConnectionHandler.AppendAsync`), which an earlier revision of this row denied. The capability nevertheless stays behind a flag that is off, and two things must land before it can be turned on. Literals are accepted **only** for `APPEND` — see the RFC 3501 row above. And the 4096-octet cap that is the whole difference between `LITERAL-` and `LITERAL+` is enforced nowhere: `MaxAppendOctets`, sized for a message, is the only bound a non-synchronising literal meets. Advertising the atom today would promise a client general literal support this server does not have, and promise it a cap this server does not apply |
