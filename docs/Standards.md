...
| Standard | RFC | Status | Milestone | Notes |
|---|---|---|---|---|
| IMAP4rev1 | 3501 | Partial | 10 | UID and UIDVALIDITY correctness is the priority. `CAPABILITY`, `NOOP`, `LOGOUT`, `STARTTLS`, `LOGIN`, `AUTHENTICATE`, `SELECT`, `EXAMINE`, `LIST`, `LSUB`, `STATUS` and `FETCH` are implemented; `FETCH` serves the stored-column data items and refuses the ones needing a MIME reader by name. Everything else is answered with a tagged `NO` naming the command rather than pretended at |
